# src/loader.py
# ─────────────────────────────────────────────────────────────────────────────
# Sets up the ontology constraints and loads the complete Hollywood dataset
...
CONSTRAINTS = [
    ...
    "CREATE CONSTRAINT hw_movie   IF NOT EXISTS FOR (m:Movie)           REQUIRE m.title IS UNIQUE",
    "CREATE CONSTRAINT hw_prod    IF NOT EXISTS FOR (p:ProductionHouse) REQUIRE p.name  IS UNIQUE",
    "CREATE CONSTRAINT hw_award   IF NOT EXISTS FOR (a:Award)           REQUIRE a.name  IS UNIQUE",
]
    ...
    for m in MOVIES:
        db.write("""
            ...
            SET n.year = $year, n.genre = $genre, n.language = $language,
                n.box_office_million = $box_office_million,
...
def load_directed(db: Neo4jConnection) -> None:
    ...
    for director, movie in DIRECTED:
        # Auto-create director node if not already in PEOPLE (e.g. James Cameron)
...
if __name__ == "__main__":
    with Neo4jConnection() as db:
        ...
        print("\n✓ Hollywood Knowledge Graph loaded.")
