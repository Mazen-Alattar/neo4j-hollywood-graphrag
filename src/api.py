# src/api.py
# ─────────────────────────────────────────────────────────────────────────────
# FastAPI backend for the Hollywood GraphRAG system.
#
# Endpoints:
...
app = FastAPI(
    title="Hollywood GraphRAG API",
    description="Knowledge graph-powered question answering over Hollywood cinema",
    version="1.0.0",
)
...
class QuestionRequest(BaseModel):
    question: str = Field(..., min_length=5, example="Which films did Leonardo DiCaprio and Hans Zimmer work on together?")
...
def ask(req: QuestionRequest):
    """
    Answer a natural language question about Hollywood using GraphRAG.
    ...
    4. Call Groq LLM to generate a natural language answer from that context
    """
    try:
...
def list_movies(limit: int = Query(default=20, le=100)):
    ...
    rows = db.read("""
        ...
        RETURN m.title AS title, m.year AS year,
               m.genre AS genre, m.box_office_million AS million
