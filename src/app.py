# src/app.py
# ─────────────────────────────────────────────────────────────────────────────
# Streamlit frontend for the Hollywood GraphRAG system.
#
# Pages:
...
if page == "💬 Chat":
    st.header("💬 Ask Anything About Hollywood")
    st.caption("Powered by a Neo4j knowledge graph + Groq LLM | Graph facts, zero hallucination")
    ...
    with st.expander("✨ Suggested questions", expanded=True):
        suggestions = [
            ...
            "Which films in the graph have crossed 500 million at the box office?",
            "Tell me about the career of Christian Bale based on graph data.",
        ]
        cols = st.columns(2)
    ...
    # Chat input
    user_input = st.chat_input("Ask about Hollywood films, actors, directors...")
...
# ─────────────────────────────────────────────────────────────────────────────
    ...
    with col1:
        entity_name = st.text_input("Name of entity", placeholder="e.g. Leonardo DiCaprio, Inception, Hans Zimmer")
    with col2:
    ...
    search_q = st.text_input("Search by topic or keyword", placeholder="e.g. space travel, revenge story, dreams")
...
# ─────────────────────────────────────────────────────────────────────────────
    ...
    st.header("🎬 Hollywood Films in the Graph")
    ...
    if data:
        ...
        min_million = col2.number_input("Minimum box office ($ million)", value=0, step=50)

        filtered = [
            m for m in movies
            if (not selected_genres or m["genre"] in selected_genres)
            and (m["million"] or 0) >= min_million
        ]

        st.caption(f"Showing {len(filtered)} films")

        for m in filtered:
            million = m["million"] or 0
            with st.expander(f"🎬 **{m['title']}** ({m['year']}) — ${million:.0f}M"):
                col_a, col_b = st.columns(2)
                col_a.write(f"**Genre:** {m['genre']}")
                col_b.write(f"**Box Office:** ${million:.0f} million")
