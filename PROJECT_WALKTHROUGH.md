# Hollywood GraphRAG — Complete Project Walkthrough
...
> *"Which music composers have worked on films that won both an Academy Award and a BAFTA Award?"*

To answer this you need to:
1. Find music composers (entities)
2. Find which films they worked on (relationship traversal)
3. Find which of those films won Academy Awards (another relationship)
4. Cross-check which of those also won BAFTA Awards (filtering)
...
User types question
   ...
   graphrag.py — Stage 3: serialise subgraph to structured text
        │
        ▼
   graphrag.py — Stage 4: send text to Groq LLM → natural language answer
        │
        ▼
...
Cillian Murphy  ──[ACTED_IN]──►  Oppenheimer
                                   │
                                   ▼
                              [WON]──► Academy Award Best Actor 2024
```
...
### `src/data/hollywood_data.py` — The raw dataset

This is just Python data — no database logic. Five lists:

| List | What it contains |
|---|---|
| `PEOPLE` | 41 people (actors, directors, composers) as dicts |
| `MOVIES` | 25 movies with descriptions and box office figures |
| `PRODUCTION_HOUSES` | 15 studios |
| `AWARDS` | 25 awards (Academy, Saturn, BAFTA, Golden Globe, etc.) |
...
```cypher
MERGE (n:Person {name: "Leonardo DiCaprio"})
SET n.born = 1974, n.profession = "Actor"
```
...
```cypher
CREATE CONSTRAINT hw_person IF NOT EXISTS FOR (p:Person) REQUIRE p.name IS UNIQUE
```
...
[1/2] Loading nodes
  ✓ 41 Person nodes
  ✓ 25 Movie nodes
  ✓ 15 ProductionHouse nodes
  ✓ 25 Award nodes

[2/2] Loading relationships
  ✓ 38 ACTED_IN relationships
  ✓ 21 DIRECTED relationships
  ✓ 16 COMPOSED_MUSIC_FOR relationships
  ✓ 18 PRODUCED relationships
...
Example:
```
"Leonardo DiCaprio is an actor born in 1974"  →  [0.12, -0.87, 0.34, 0.05, ..., 0.91]  (768 numbers)
"Tom Hardy is an actor born in 1977"        →  [0.15, -0.82, 0.39, 0.02, ..., 0.88]  (768 numbers)
"The Eiffel Tower is in Paris"              →  [-0.71, 0.23, -0.55, 0.91, ..., -0.12] (768 numbers)
```
...
```python
# For a Movie node:
"'Inception' is a Science Fiction Thriller English film released in 2010.
 A skilled thief enters dreams to steal secrets and is offered a chance..."

# For a Person node:
"Leonardo DiCaprio is an actor, director, or composer born in 1974 from Los Angeles."

# For a ProductionHouse node:
"Warner Bros. Pictures is a film production company founded in 1923 by Warner brothers."
```

This matters a lot. A poorly written sentence = a bad embedding = wrong nodes retrieved for a question.

### Step 2 — Call Google GenAI's embedding API

```python
response = client.models.embed_content(
    model="text-embedding-004",
    contents=["'Inception' is a Science Fiction..."]
)
vector = response.embeddings[0].values  # list of 768 floats
```

`text-embedding-004` is Google's state-of-the-art embedding model. It converts any text into a 768-dimensional vector.
...
Neo4j Node: Movie {
    ...
    box_office_crore: 829.9,
    description: "A skilled thief enters dreams...",
    embedding: "[0.12, -0.87, 0.34, ...]",   ← stored as JSON string
    embedding_text: "'Inception' is a Science Fiction..."
}
```
...
q_vec = client.models.embed_content(model="text-embedding-004", contents=["Which films crossed 1000 million?"]).embeddings[0].values
...
collection = chroma.get_or_create_collection("hollywood_nodes")
...
**For this project, Neo4j JSON storage is the right call** — the graph has ~106 nodes. The simplicity of one system beats the added complexity of a second database.
...
```
Question: "Which films did Leonardo DiCaprio and Hans Zimmer collaborate on?"
    ↓ embed
[0.23, -0.54, 0.87, ...]  (768-dim vector)
    ↓ cosine similarity against all node embeddings
    ↓ top-3 results:
  [Person] Leonardo DiCaprio   score=0.91
  [Person] Hans Zimmer         score=0.88
  [Movie]  Inception           score=0.82
```
...
```
Question: "dream heist and space travel"
→ finds: Movie "Inception" (score=0.89)  ← no name mentioned, pure concept match
```
...
```cypher
MATCH (start:Person)
WHERE start.name = "Leonardo DiCaprio"
...
```python
{
    "center": {"name": "Leonardo DiCaprio", "born": 1974, "profession": "Actor", ...},
    "label": "Person",
    "edges": [
        {"from": "Leonardo DiCaprio", "rel": "ACTED_IN",           "to": "Inception"},
        {"from": "Leonardo DiCaprio", "rel": "ACTED_IN",           "to": "The Revenant"},
        {"from": "Tom Hardy",         "rel": "ACTED_IN",           "to": "Inception"},
        {"from": "Hans Zimmer",        "rel": "COMPOSED_MUSIC_FOR", "to": "Inception"},
        {"from": "Inception",          "rel": "WON",                "to": "Academy Award Best Visual Effects 2011"},
        ...
    ]
}
```
...
```
ENTITY: Leonardo DiCaprio [Person]
Properties: born=1974, profession=Actor, hometown=Los Angeles
...
CONNECTIONS:
  ...
  • Inception          –[WON]→  Academy Award Best Visual Effects 2011
...
LLMs work on text. The bullet-point format with entity/relationship labels is clear, unambiguous, and easy for the LLM to parse and reason over.
...
```python
SYSTEM_PROMPT = """You are an expert on Hollywood cinema...
...
Separation of concerns. The Streamlit app only knows HTTP — it doesn't need a database driver or Groq/Gemini API keys. This means:
...
Browse all 25 movies with genre and box-office filters. Each movie can expand to show its full graph context (cast, director, composer, awards, production house).
...
**Triple**  
The fundamental unit of a knowledge graph fact: (Subject) → [Predicate] → (Object). E.g.: Leonardo DiCaprio → ACTED_IN → Inception.
